...
sys_exofork(void)
{
    ...
    return e->env_id;
	//panic("sys_exofork not implementedl;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    {
        if(((int32_t)srcva%PGSIZE != 0)||((perm&(~PTE_SYSCALL))||((perm&PTE_U) ==0)||((perm&PTE_P) == 0))
        )
        {
            return -E_INVAL;
        }
        pg = page_lookup(curenv->env_pgdir,srcva,&pte_store);
        if(pg == NULL||pte_store==NULL) return -E_INVAL;
        if((perm&PTE_W)&&((*pte_store&PTE_W)==0))
        {
            return -E_INVAL;
        ...
        {
            res = page_insert(dste->env_pgdir,pg,dste->env_ipc_dstva,perm);
            if(res < 0) return res;
        }
...
int32_t
syscall_wrapper(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5,uint32_t aesp,uint32_t aeip)
{
    int res;
    lock_kernel();
